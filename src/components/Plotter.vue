...
let pathLine = null, startMarker = null, headMarker = null
let descentPath = null, shown = 0
...
useThreeViewport(hostRef, {
  ...
  onReady: (manager) => {
    ...
    sm.scene.add(mesh, axesGroup, critMarker, pathLine, startMarker, headMarker)
...
function update() {
  if (!descentPath || shown >= descentPath.length) return
  shown = Math.min(descentPath.length, shown + 0.5)
  const drawn = Math.floor(shown)
  const pts = []
  for (let i = 0; i < drawn; i++) {
    const p = descentPath[i]
    pts.push(new THREE.Vector3(worldX(p.x), wy(p.z) + 0.03, worldX(p.y)))
  }
  if (pts.length) {
    pathLine.geometry.setFromPoints(pts)
    headMarker.position.copy(pts[pts.length - 1]); headMarker.visible = true
    const sp = descentPath[0]
    startMarker.position.set(worldX(sp.x), wy(sp.z) + 0.03, worldX(sp.y)); startMarker.visible = true
  }
...
async function runDescent() {
  try {
    ...
    console.error('Plotter descent failed:', e)
  }
}
...
watch([fn, range, resolution], () => { resetDescent(); recompute() })
  ...
  <AlgorithmLayout
    ...
    <template #controls>
      ...
      <ControlPanel number="02" title="Gradient descent">
        <RangeField v-model="startX" :min="-3.5" :max="3.5" :step="0.1" label="Start · x₀" :format="(v) => v.toFixed(2)" />
        <RangeField v-model="startY" :min="-3.5" :max="3.5" :step="0.1" label="Start · y₀" :format="(v) => v.toFixed(2)" />
        ...
        <div class="btnrow">
          <AppButton @click="runDescent">Run descent →</AppButton>
